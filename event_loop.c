...
/* To remove or detach the first task that is present at the head of
 * the task array in this event loop object
 * and return the particular task
 */
...
/*
 * append a new task at the end of task array of the event loop object
 * task array is a doubly linked-list
 */
...
/* check if a task is present in the task array */
...
static void *event_loop_thread(void *arg) {
    ...
    while(1) {
        ...
        /* Event Loop thread do not have any task to fire, suspended the event loop thread */
           if (el_debug) {
           ...
           /* We are here when event loop thread receives signal. On receiving the
...
void event_loop_run(event_loop_t *el) {
    ...
    // detach mode thread runs in background. It is optional
