...
static int stp_update_routing_table(rt_table_t *rt_table, uint32_t cmd_code, rt_table_entry_t *rt_entry) {
...
static void pkt_process_fn(char *msg_recvd, uint32_t msg_size, char *sender_ip, uint32_t port_no, uint32_t fd) {

    printf ("route recvd on port no %u from IP %s\n", port_no, sender_ip);
...
cli_handler(int choice)
{
    ...
    case 5:
    {
        ...
        scanf("%d", &portno);
        udp_server_create_and_start("127.0.0.1", portno, pkt_process_fn);
    }
    break;
    case 6:
        exit(0);
...
int main(int argc, char **argv){
    ...
    if (!rt_table) {
        rt_table = rt_create_new_rt_table("Table1");
