...
extern int stp_update_routing_table(rt_table_t *rt_table, uint32_t cmd_code, rt_table_entry_t *rt_entry);

void stp_init_el(event_loop_t *el) {

    event_loop_init(el);
    event_loop_run(el);
}

static void el_stp_update_routing_table_cbk(void *arg) {

    el_rt_table_update_data_t *el_rt_table_update_data = (el_rt_table_update_data_t *)arg;
...
task_t *el_stp_update_routing_table(rt_table_t *rt_table, int cmd_code, rt_table_entry_t *rt_entry) {
    ...
    memcpy((char *)el_rt_table_update_data->rt_entry, rt_entry, sizeof(*rt_entry));

    // submit the computation to the event loop - computation function prototype void (*)(void*)
    task_t *task = task_create_new_job (&el, el_stp_update_routing_table_cbk, (void *)el_rt_table_update_data);
